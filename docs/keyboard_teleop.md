...
RBY-1 启动后默认控制左臂。按 `Tab` 可按以下顺序循环切换控制模式：

```text
left_arm -> right_arm -> base -> left_arm
```

相机窗口会同时显示头部、左腕和右腕画面，左上角标明当前控制模式。
...
| `←` / `→` | 当前末端沿局部 **+Y / -Y** 移动 |
| `S` / `W` | 当前末端沿局部 **+Z / -Z** 移动 |
| `Z` / `C` | 当前末端绕 X 轴正向 / 反向旋转（roll） |
| `E` / `R` | 当前末端绕 Y 轴正向 / 反向旋转（pitch） |
| `A` / `D` | 当前末端绕 Z 轴正向 / 反向旋转（yaw） |
| `Space` | 切换当前手臂夹爪的开合状态 |
| `Q` | 结束当前 episode |

逆运动学只移动当前选择的机械臂；另一只机械臂、躯干和底盘保持不动。
...
底盘模式下 `W/S`、`E/R`、`Z/C` 不产生动作。`Space` 也不会操作夹爪；请先切换到对应的机械臂模式。
...
- **RBY-1 夹爪没有响应**：确认当前模式是 `left_arm` 或 `right_arm`；底盘模式会忽略 `Space`。
- **RBY-1 底盘没有响应**：确认相机画面左上角显示 `Control: base`。默认 RBY-1 配置使用支持横移的全向底盘。
